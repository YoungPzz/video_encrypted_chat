...
namespace mediasoupclient
{
	...
	/**
	 * Sets the frame encryptor for the RTP sender.
	 */
	void Producer::SetFrameEncryptor(webrtc::FrameEncryptorInterface* encryptor)
	{
		MSC_TRACE();
		...
		// 获取底层的 RtpSender 并设置加密器
		if (this->rtpSender != nullptr)
		{
			this->rtpSender->SetFrameEncryptor(encryptor);
			MSC_DEBUG("Frame encryptor set on RTP sender: %p", encryptor);
		}
		else
		{
			MSC_WARN("RTP sender is null, cannot set frame encryptor");
		}
	}
