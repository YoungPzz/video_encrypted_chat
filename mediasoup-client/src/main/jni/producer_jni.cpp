...
static void JNI_Producer_SetFrameEncryptor(JNIEnv* env, jlong producer, jlong encryptor) {
	MSC_TRACE();
	try
	{
		// 1. 类型转换：将 JNI 层的 long 指针转换为 Native 层的 Producer 指针
		auto* owned_producer = reinterpret_cast<OwnedProducer*>(producer);

		// 2. 类型转换：将 JNI 层的 encryptor 指针转换为 FrameEncryptorInterface 指针
		auto* frame_encryptor = reinterpret_cast<webrtc::FrameEncryptorInterface*>(encryptor);

		// 3. 调用 Producer 的 SetFrameEncryptor 方法
		owned_producer->producer()->SetFrameEncryptor(frame_encryptor);
	}
	catch (const std::exception& e)
	{
		// 4. 异常捕获与错误处理
		MSC_ERROR("failed to set frame encryptor: %s", e.what());
